import { MedrxivSearcher } from "../providers/medrxiv.js";
import fs from "fs";

describe("MedrxivSearcher", () => {
  const searcher = new MedrxivSearcher();
  const testSavePath = "./downloads";

  it("should search medRxiv and return papers", async () => {
    const results = await searcher.search("cardiovascular medicine", 2, 7);
  ...
  it("should download a PDF for a known paper", async () => {
    // Use a known medRxiv DOI for testing
